...
export default {
  ...
  sockets: {
    ...
    // TODO : Rest of listener : (newUser, privateChat, leavePrivateRoom, leaveChat)
  },
  beforeCreate: function() {
    this.$sockets.emit(WS_EVENTS.joinRoom, this.$store.state); // Join the room
  },
  data: function() {
    return {
  ...
  methods: {
    sendMessage(msg) {
      ...
      this.$sockets.emit(WS_EVENTS.publicMessage, {
        ...this.$store.state,
        messages: msg
      });
    },
    // TODO : Rest of the methods (onChangeRoom, openChat, closePrivateChat, logout)
  }
}
</script>
